...
public final class WriterAgent extends ArcaneAgent {
    public WriterAgent(AccessRequest d, List<Grimoire> g, List<SyncCoordinator> c, SimulationConfig conf, EventBus eb) {
        super(d, g, c, conf, eb);
    }
    @Override protected Duration activityDuration() { return RandomDuration.between(config().minWrite(), config().maxWrite()); }
    @Override protected void enterCriticalRegion() { publish(EventType.STATE, MageState.WRITING, grimoire().write(descriptor())); }
}